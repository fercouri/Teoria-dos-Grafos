...
public:
    ...
    void exportarDOT(const std::string& nomeArquivo) const;
    ...
    Grafo* guloso(); // j
    Grafo* guloso_rando(float alpha); // k
    Grafo* guloso_rando_reativo(); // l

    bool adjacente_a_conjunto(char v, const set<char>& conjunto); //k 1
    bool eh_ids(const std::set<char>& conjunto) const;
